...
6. **`ArrayBuffer` через `chrome.tabs.sendMessage` (SW → content)** → не доживает:
   JSON-сериализация превращает его в пустой объект, и `new Blob([{}])` даёт файл
   [object Object] (15 байт). Байты надо передавать как обычный массив чисел
   (`Array.from(new Uint8Array(buf))`) и собирать обратно `new Uint8Array(arr)` в MAIN-мире.
